...
const props = defineProps<{ champion: Champion | null; status: Status | null }>()
...
const augBy = (b: Build, r: string) => (b.augments ?? []).filter((a) => a.rarity === r).slice(0, 8)
...
<template>
  <section class="panel">
    <div v-if="!champion" class="muted">Pick a champion on the left. When you lock in during champ select the build shows here automatically.</div>
    <template v-else-if="build">
      <div class="build-head">
        <img :src="build.champion.image" :alt="build.champion.name" />
        <div>
          <div class="name">{{ build.champion.name }}</div>
          <div class="title">{{ build.champion.title }} · patch {{ build.patch }}<span v-if="build.mode === 'aram'"> · Howling Abyss</span></div>
        </div>
        <div class="grow" />
        <div class="roles">
        ...
        <div class="roles" v-if="build.mode !== 'aram'">
      ...
      <div class="row" style="margin-top: 10px">
        <span class="badge" :class="build.source">
          {{ build.source === 'opgg' ? 'ARAM stats · op.gg · patch ' + build.patch.replace('-aram', '') : build.source === 'riot' ? 'Compiled from ranked matches' : build.source === 'lcu' ? 'Riot in-client recommendations' : 'No data' }}
        </span>
        ...
        <label class="toggle" :class="{ on: follow }" style="margin-left: auto" @click="follow = !follow">
          <span class="track" /><span class="muted">Follow champ select</span>
        </label>
      </div>
      ...
      <template v-if="build.augments?.length">
        <h3>
          Mayhem augments
          <span class="muted" style="text-transform: none; letter-spacing: 0">{{ build.augScope === 'champion' ? `${build.champion.name}-specific stats` : 'global stats' }} · aramgg.com</span>
        </h3>
        <div class="augs">
          <div v-for="r in RARITIES" :key="r" class="aug-col" :class="r">
            <div class="aug-head">{{ r }}</div>
      ...
      <div class="grid2">
        ...
        <div>
          <template v-if="build.runes?.length">
            <h3>Runes <span v-if="applied" class="muted" style="text-transform: none; letter-spacing: 0">{{ applied }}</span></h3>
            <div class="runes">
...
.augs { display: grid; grid-template-columns: repeat(3, 1fr); gap: 12px; }
@media (max-width: 1000px) { .augs { grid-template-columns: 1fr; } }
.aug-head { font-family: var(--display); font-size: 11px; letter-spacing: .16em; text-transform: uppercase; padding: 4px 8px; margin-bottom: 6px; border: 1px solid; }
.aug-col.prismatic .aug-head { color: #c7f3ff; border-color: #3fb4d8; background: linear-gradient(90deg, rgba(63,180,216,.18), transparent); }
.aug-col.gold .aug-head { color: var(--gold-bright); border-color: var(--gold); background: linear-gradient(90deg, rgba(200,170,110,.18), transparent); }
.aug-col.silver .aug-head { color: #d8dde3; border-color: #8b9bb0; background: linear-gradient(90deg, rgba(139,155,176,.18), transparent); }
