...
<template>
  <section class="panel" v-if="cs?.active">
    <h2>
      Champ select
      <span class="muted" style="margin-left: 10px">{{ cs.mode === 'aram' ? 'ARAM' : cs.phase }} · {{ cs.timeLeft }}s</span>
      <label class="toggle" :class="{ on: autoRunes }" style="margin-left: auto" title="Set Riot's recommended rune page automatically for the champion you get" @click="autoRunes = !autoRunes">
        <span class="track" /><span>Auto runes</span>
      </label>
    </h2>
    <div v-if="runeMsg" class="muted" style="margin: -6px 0 8px">{{ runeMsg }}</div>
    <div class="teams">
      <div>
        <h3>Your team</h3>
        <div class="team">
          <div
            v-for="p in cs.myTeam ?? []"
            :key="p.name + p.champion.id"
            class="cs-player"
            :class="{ me: p.isMe }"
            @click="p.champion.id && emit('preview', p.champion)"
          >
    ...
    <template v-if="cs.benchEnabled">
      <h3>
        Bench
        <button class="primary" style="margin-left: auto" :disabled="busy || cs.rerollsRemaining <= 0" @click="act(api.reroll)">
          Reroll ({{ cs.rerollsRemaining }})
        </button>
      </h3>
